...
data "aws_iam_policy_document" "lambda_instana" {
  statement {
    actions = [
...
resource "aws_iam_role_policy" "lambda_instana" {
  policy = data.aws_iam_policy_document.lambda_instana.json
  role   = data.terraform_remote_state.vault_aws_auth.outputs.lambda_role_id
}
...
resource "aws_lambda_function" "this" {
  ...
  runtime          = "python3.12"
  handler          = "instana.lambda_handler" # Custom handler for the Lambda
  timeout          = 60
  memory_size      = 128
  ...
  layers           = [
    ...
    "arn:aws:lambda:${var.region}:410797082306:layer:instana-python:72"
  ]
